function submitForm(event) {
    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            $("#result").html('Method: ' + data.method);
            $("#function").html('f(x): ' + getFunctionTextRepresentation(data));
            $('#standard_deviation').html('Standard deviation: ' + data.standardDeviation.toFixed(5))
            $('#phis').html('Phis: ' + data.phiList.map(e => e.toFixed(5)).join(' '))
            $('#epsilons').html('Epsilons: ' + data.epsilonList.map(e => e.toFixed(5)).join(' '))
            $('#pirson').html('Pirson coefficient: ' + data.pirsonCoefficient.toFixed(5))
...
function processGraphs(methodResults, min_x, max_x, min_y, max_y, points) {
    for (let i = 1; i <= 5; i++) {
        ...
        if (data.approximationFunction.length === 0) {
            $('#result_' + i).html('Method: ' + data.method);
            $('#function_' + i).html('Can not approximate function with this method');
            ...
            $('#standard_deviation_' + i).html('Standard deviation: ' + data.standardDeviation.toFixed(5))
            $('#phis_' + i).html('Phis: ' + data.phiList.map(e => e.toFixed(5)).join(' '))
            $('#epsilons_' + i).html('Epsilons: ' + data.epsilonList.map(e => e.toFixed(5)).join(' '))
            $('#pirson_' + i).html('Pirson coefficient: ' + data.pirsonCoefficient.toFixed(5))
        }
